...
3. Сборка не подписана сертификатом Apple, поэтому при первом запуске macOS скажет,
   что приложение повреждено или от неизвестного разработчика. Снимите карантин один раз:
   ```
   xattr -dr com.apple.quarantine /Applications/orca-board.app
   ```
   или правый клик по приложению → «Открыть» → «Открыть».
...
Роли и колонки: у проекта есть роли (агент + модель, `orca-board roles list`) и настраиваемые
колонки доски (`orca-board columns list`); задача создаётся с `--role`, статус — id колонки.
Модель роли уходит агенту флагом (`claude --model`, `codex -m` и т.д.). Карточки сортируются
