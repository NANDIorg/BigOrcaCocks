...
export interface ProjectTemplate {
  ...
  description?: string
  /**
   * Встроенный шаблон из кода: только для чтения (его можно дублировать; без копии меняются лишь агент, модель и
   * усилие ролей — `isBuiltinExecutorEdit`) и обновляется вместе с приложением,
...
/**
 * Поля роли, которые можно менять у встроенного шаблона без «Дублировать»: исполнитель — агент и зависящие от него
 * модель и усилие (у другого агента свои модели, поэтому агент меняется вместе с ними).
 */
export const BUILTIN_EDITABLE_ROLE_FIELDS = ['agent', 'model', 'effort'] as const
 ...
 * ролей, те же, а у ролей (тот же состав и порядок) отличаются только агент, модель и усилие. Так «Настройки» дают
 * сменить исполнителей встроенного шаблона, не открывая его целиком — остальное правится через «Дублировать».
 */
export function isBuiltinExecutorEdit(
