...
import { BUILTIN_TEMPLATES, GENERAL_TEMPLATE_ID, builtinTemplate, builtinTemplates, isBuiltinExecutorEdit } from './templates.ts'
...
describe('isBuiltinExecutorEdit — правка встроенного шаблона без копии', () => {
  const frontend = builtinTemplate('frontend')!
  const general = builtinTemplate(GENERAL_TEMPLATE_ID)!
  const roles = (t: typeof frontend) => (t.settings.roles ?? DEFAULT_ROLES).map((r) => ({ ...r }))

  it('агента, модель и усилие ролей можно менять, в том числе у шаблона без явных ролей', () => {
    for (const t of [frontend, general]) {
      const next = roles(t)
      next[0] = { ...next[0], model: 'opus', effort: 'high' }
      next[1] = { ...next[1], agent: next[1].agent === 'claude' ? 'codex' : 'claude', model: 'gpt-5' }
      assert.equal(isBuiltinExecutorEdit(t, { title: t.title, description: t.description, settings: { ...t.settings, roles: next } }), true, t.id)
    }
  })

  it('без изменений — тоже можно (роли записаны явно)', () => {
    assert.equal(isBuiltinExecutorEdit(general, { title: general.title, description: general.description, settings: { ...general.settings, roles: roles(general) } }), true)
  })
  ...
  it('остальное — только через копию', () => {
    ...
    const cases = [
      ...
      { ...base, settings: { ...frontend.settings, roles: roles(frontend).map((r, i) => (i === 0 ? { ...r, systemPrompt: 'x' } : r)) } },
      { ...base, settings: { ...frontend.settings, roles: roles(frontend).slice(1) } },
      { ...base, settings: { ...frontend.settings, roles: roles(frontend).reverse() } }
    ]
    for (const c of cases) assert.equal(isBuiltinExecutorEdit(frontend, c), false, JSON.stringify(c).slice(0, 80))
  })
})
