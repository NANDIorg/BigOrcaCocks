...
/** Цель, если человек вставил только изображения без текста. */
export const DEFAULT_IMAGE_OBJECTIVE = 'Разбери приложенные изображения и выполни то, что на них показано.'
...
export function coordinatorPrompt(objective: string, imagePaths: readonly string[] = []): string {
  ...
  if (imagePaths.length > 0) {
    parts.push(
      [
        `К цели приложены изображения (${imagePaths.length}) — это часть задания. Прежде чем декомпозировать,`,
        'открой и посмотри каждое инструментом чтения файлов (Read), пути абсолютные:',
        ...imagePaths.map((p) => `- \`${p}\``),
        'Файлы хранятся до конца прогона. Если изображение нужно воркеру — укажи этот путь в описании его задачи.'
      ].join('\n')
    )
  }
