...
самого orca-board — [git-flow.md](git-flow.md). Нода `merge` сливает ветку задачи в ветку её глобальной задачи
(«Ветка глобальной задачи» в `docs/architecture.md`), а без неё — в текущую ветку root, если та не защищённая. Это
локальный мерж подзадачи: он не создаёт GitHub PR, не проверяет CI и не заменяет ревью второго разработчика. PR ветки
глобальной задачи открывает не воркфлоу, а `RunBranchSync` после push, если включена настройка `pr`.
