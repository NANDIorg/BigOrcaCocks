...
|---|---|---|---|---|
| `create_branch` | создаёт ветку `branch` от `base` и переключает на неё worktree | `branch` | `base` | `base` — текущая ветка корня репозитория (та, куда сольёт `merge`); если worktree задачи уже есть — то место, где он стоит (коммиты задачи не теряются) |
