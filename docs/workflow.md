...
- **Подзадача этапа `work`** (`Task.stageOf {nodeId, visit}`) — обычная рабочая задача: воркер → `done` → автомерж в ветку прогона; идёт по пути подзадачи (`work.subflow` или `defaultSubflow()`), позиция — `Task.stage` на ноде пути (см. раздел «Путь подзадачи»).
