...
Редактор пути подзадачи (вход в ноду, крошки, подсветка проблем по `nodeId`-пути) — `docs/architecture.md`, «Воркфлоу: редактор».
...
не читается. Версий у шаблона нет. Хранение и IPC — «Библиотека шаблонов» ниже, редактор (палитра «Свои ноды», «Сохранить как свою ноду», «Обновить из шаблона» — сверка ноды с шаблоном по содержимому, а не по `updatedAt`) — docs/architecture.md → «Воркфлоу: редактор»; контракт core — модуль `node-templates.ts` (без node-импортов: его читает renderer):
...
стоит на `gate` или `human` своего пути (конфликт мержа — тоже `human`): пока она не дойдёт до `end`, этап не закроется (`stage_tasks_done`). В карточке задачи — блок «Путь подзадачи»: этап, шаг,
«держит этап» и история шагов по `Task.stageHistory` (вход в шаг, заход, заметный исход — возврат, конфликт, время на шаге); полоса «По этапам» статистики подписывается теми же названиями.
Карточка глобальной задачи не меняется — там по-прежнему этап прогона (`Run.stage`). Без `workflowScope: 'run'`, графа или `Task.stageOf` (старый main, прогон старого формата) блоков нет.
