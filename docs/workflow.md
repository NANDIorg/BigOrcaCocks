...
автомерж в ветку глобальной задачи (конфликт — запрос человеку). Раздел описывает контракт слоя `packages/core`, движок
main (`apps/desktop/src/main/workflow-run.ts`, «Движок main, промпты и skills прогона»), skills и промпты, сокет/CLI (`docs/architecture.md`) и renderer («Renderer» ниже).
...
из `validateWorkflow` с переводом по `code`, импорт графа v1 показывает, что снято миграцией, а замечания автомиграции сохранённого типа (`TaskType.workflowNotes`) стоят над проблемами графа до «Понятно» (`patch` с `workflowNotes: []`). Экран глобальной задачи: чип «Этап: …» (`runStageLabel`), входы в этапы
...
`task create` (чужая роль, не «Работа»), автомерж подзадач в ветку прогона (корень не тронут), `stage_tasks_done` один раз на заход, `stage finish` → задача-проверка по ветке прогона,
...
`run_done` ровно один раз на `end`, старый прогон рядом идёт прежним движком, перезапуск приложения на approval с мёртвым координатором. Конфликты мержа (ветка прогона и подзадача), `git`-ноды,
...
- **Подзадача этапа `work`** (`Task.stageOf {nodeId, visit}`) — обычная рабочая задача: воркер → `done` → автомерж в ветку прогона; по графу не ходит, своего этапа у неё нет.
