...
(конфликт — запрос человеку). Раздел описывает контракт слоя `packages/core` и, ниже, движок
main (`apps/desktop/src/main/workflow-run.ts`, «Движок main, промпты и skills прогона»); сокет/CLI, skills и промпты уже сделаны поверх контракта, renderer — отдельная задача.
...
с наследованием инструкций, показа и ролей внешней «Работы». Автозакрытие старого движка (`closeFinishedRuns`, `run_done` «все подзадачи
...
«Конфликт мержа» (`accept` → снова `merge`, `reject` → `work`). Это тот же путь, что прежде был зашит в движке прогона (`subtaskDone` → `mergeSubtask`); подзадача
...
подзадачи — позиция **внутри пути**, `Task.stageOf` — какой «Работе» путь принадлежит; `Run.stage` независим, стека позиций нет. События `stage_changed` и
...
удалять**. Подключение его к движку прогона (маршрутизация событий, замена `SUBTASK_MERGE_NODE`, рестарт) — отдельная задача; пока `subtaskDone` по-прежнему мержит сам.
