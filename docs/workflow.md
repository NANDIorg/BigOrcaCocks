...
из `validateWorkflow` с переводом по `code`, импорт графа v1 показывает, что снято миграцией. Экран глобальной задачи: чип «Этап: …» (`runStageLabel`), входы в этапы
...
Сокет и CLI (`stage finish`, `workflow show` по прогону, `global get` → `stage`, `review accept|reject` по проверке прогона, ошибки `task create`, `runs finish` для
`scope: 'run'`), промпты и skills координатора и воркера, автомиграция сохранённых графов типов при загрузке (`migrateWorkflowReport`, замечания в
`TaskType.workflowNotes`) — сделаны; протокол — «Протокол сокета» и «CLI» в `docs/architecture.md`. Осталось отдельными задачами:

- **Движок main:** эффекты нод (создание задач-проверок и вопросов, слияние прогона в базу, автомерж подзадач, подписки на решения). Сокет только двигает граф
  (`finishStage`, `advanceRunStage`); до выхода движка main и renderer работают со старым форматом: там, где запрос или проверка
  могут не иметь задачи, стоят только защитные проверки типов.
- **Возврат в работу** (`returnToWork` в `main/worker.ts`) для `scope: 'run'` пока гасит живого координатора и запускает нового, хотя координатор прогона с воркфлоу
  должен жить до конца графа: доработка — в движке main; renderer терминал показывает, если он вернулся.
- **Замечания автомиграции сохранённых типов** (`TaskType.workflowNotes`) renderer пока не показывает — только при импорте графа файлом; вывод в редакторе типа — отдельная задача renderer.
