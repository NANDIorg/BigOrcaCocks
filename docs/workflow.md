...
| `ask` | задача в ready, запуск агента (`runWorker`); вопросы агента адресуются человеку (запрос `question` с `nodeId` ноды, задача в «Нужен ответ»); роль ноды **не** становится ролью задачи | `orca-board done` агента → `next` |
...
Исполнитель (запуск агента, адресация вопроса, автоперезапуск после ответа) и промпт — отдельными шагами прогона «Нода ask».
