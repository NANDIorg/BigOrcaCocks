...
`workflow show` по прогону, `review` по проверке прогона, ошибки `task create`), сообщение человеку о снятых нодах при автомиграции сохранённых графов типов
(`migrateWorkflowReport` при загрузке `projects.json`; renderer сейчас показывает замечания только при импорте файла), промпты и
