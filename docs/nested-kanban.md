...
подзадачи и не закрывает/не открывает прогон. Автоматика меняет статус карточки только в трёх случаях:
...
| Все подзадачи в `kind=done` (`closeFinishedRuns`) | `closedAt`, событие `run_done`, карточка → `kind=done` |
...
`closeRun` (ручное закрытие) статус карточки не меняет. Удаление колонки переносит и карточки
(`reassignColumn`).
...
- **Повторный запуск на существующей**: `coordinator start --global <id>` = `global start --global <id>`,
  ...
  Цель — описание (пусто → название) плюс список уже созданных подзадач `- <id> [<колонка>] <название>`
  с просьбой продолжить их, а не создавать заново. Старые вложения этой глобальной удаляются перед записью
...
- `packages/core/src/global-tasks.test.ts` — CRUD, реальные колонки, прогресс, изоляция, удаление,
  lifecycle (run_done, повторный запуск, переоткрытие), сохранение и миграция старого снапшота.
