...
**Прогон с воркфлоу глобальной задачи** (`Run.workflowScope: 'run'`, `docs/workflow.md`) живёт по графу, а не по этой таблице: автозакрытие
(`closeFinishedRuns`, `run_done` «все подзадачи в done», `settleIdleRuns`) для него выключено. Карточка следует за нодой: `work`, `ask`, `gate`, `decision`, `git`, `merge` —
«В работе» (на `decision`, пока ждёт запрос `decision` человеку — «Нужен ответ», как при любом pending-запросе прогона), `human` — «Проверка» (ждёт approval прогона; «Подтвердить» и «Вернуть в работу» решают его), `end` — прогон закрыт (`closedAt`), карточка в «Сделано»
и координатору `run_done`. Конец этапа `work` — событие `stage_tasks_done`, а не `run_done`. `runs finish` до конца графа — ошибка, после — сигнал «закончил».

Исполняет граф прогона `apps/desktop/src/main/workflow-run.ts` («Движок main» в `docs/workflow.md`): вход в граф — после запуска координатора (`runCoordinator`), «Подтвердить»
(`globalTasks:accept` → `acceptRun`) и «Вернуть в работу» (`globalTasks:returnToWork` → `returnRun`) решают approval ноды `human`, а переход по нему делает движок, а не `returnGlobalTaskToWork`:
живой координатор при возврате не закрывается — он получает `stage_started` с замечаниями, мёртвый запускается заново с блоком «# Этап» в цели (`resumeObjective`). Терминал координатора отдаётся
в IPC как раньше. Дошедший до `end` прогон координатора не запускает. `settleIdleRuns` для таких прогонов не работает; страховка на мёртвого координатора — `settleIdleRunStages`.
...
- **Повторный запуск на существующей**: `coordinator start --global <id>` = `global start --global <id>`,
  ...
  дубли не создавай. У прогона с воркфлоу глобальной задачи (`workflowScope: 'run'`) цель ещё несёт блок «# Этап: …» (`docs/workflow.md`,
...
запросов к человеку (`HumanRequest`) для старого формата не используется: pending-запрос прогона сам переводит карточку в «Нужен
ответ», а у приёмки нет подзадачи для обязательного `taskId`. Карточка в «Проверке» (как и в done) в «Нужен ответ»
не поднимается (`globalDisplayStatus`), основное время в ней стоит. У прогона с воркфлоу (`workflowScope: 'run'`) «Проверка» — это ждущий approval
ноды `human` (`HumanRequest` без `taskId`, `docs/human-requests.md`): он виден и в Инбоксе, а «Подтвердить»/«Вернуть в работу» решают его; карточка при этом остаётся в «Проверке». Уведомление — `run_done` («Подзадачи сделаны, скоро проверка: …»,
...
- `GlobalTaskView.tsx` — экран открытой глобальной задачи: `GlobalTaskHeader` (компактная шапка, см. ниже),
  лента «Ждут вас» и **вкладки «Доска · Итог и цель · Координатор · История · Статистика»**. Лента и шапка стоят над
  ...
  - `CoordinatorPanel.tsx` — вкладка «Координатор» (слева состояние и запуски, справа последний вывод; ≤ 900px — одна колонка):
    - Состояние (`coordState` в `coordPanel.ts`): нет живого PTY — «не запущен»; жив и есть pending-запросы (`GlobalTask.waiting`) или
      колонка «Нужен ответ» — «ждёт вас»; иначе «работает». Живой: «Открыть терминал» (вкладка «Терминалы») и «■ Остановить» — своё
      подтверждение в панели, затем `closeTerminal` → `pty.kill` (тот же путь, что у закрытия вкладки терминала). Не запущен:
      «Запустить координатора» (кроме «Проверки») или «Вернуть в работу…» на «Проверке».
    - Запуски — `Run.coordinatorSessions` (`snap.runs` → пропс `coordinatorSessions` в `GlobalTaskView`, тот же, что у шапки и «Истории»), `sessionRows`: новые сверху, «Запуск N», агент и модель,
      период (конец в тот же день — только время) и длительность; живой запуск — «идёт», без `endedAt` и без живого PTY — «конец
      неизвестен». Поля нет (старый main, прогон до учёта запусков) — «Запуски неизвестны», если координатор уже запускался
      (`GlobalTask.coordinatorPtyId`, `knownSessions`), и «Запусков не было», если нет; пустой список — тоже «Запусков не было».
    - Последний вывод (`coordTail.ts`) — только чтение, без второго xterm и без `resize` (размер PTY принадлежит терминалу): хвост из
      `terminals.list()` → `tail`, дальше `pty.onData` (подписка до запроса списка, `mergeTail` отсекает уже вошедшее в хвост), в панель —
      последние `COORD_TAIL_LINES` (40) строк без ANSI; `\r` затирает строку, повторы схлопываются; обновление не чаще раза в 300 мс,
      лента следует за концом, пока не прокрутили вверх. Нет `terminals`/`pty` в `window.orca` — хвост пуст.
      TUI (Claude Code) рисует экран позиционированием курсора, а не пробелами: `ESC[nG` (в колонку) и «вправо» → пробел,
      «вниз»/`ESC[n;mH` → перевод строки (`layoutToText`), иначе слова слипаются. Раскладку экрана это не восстанавливает.
  ...
  - `GlobalHistory.tsx` («История»): единая лента по дням, новые сверху (`groupByDay`), строки — время, точка цвета колонки и текст.
    ...
    У записи ноды «Решение ИИ» (`StageChange.decision`) — подпись «Решение ИИ: <метка>» или «Решил человек: <метка> · <почему человек>»,
    цитата — обоснование (одной строкой, до `DECISION_EXCERPT_LIMIT` = 400 символов), второй цитатой — комментарий агента (`agentNote`); исход
    следующей записи — id варианта — отдельно не подписывается. Нет метки — id варианта; старый main без `decision` — обычная запись этапа.
    ...
    текст целиком), «Координатор оставил сводку» (`summary`: только выдержка, полный текст — на «Итог и цель»), «Запущен координатор»
    (по событию на запуск: агент, модель, «N-й запуск», сколько работал), «Работа закрыта» (`closedAt`). Записи без корректного времени
    отбрасываются; при равной метке порядок «причина → следствие» (`KIND_RANK`). Без событий — «История пока пуста» (при
    `statusHistory === undefined` — совет перезапустить приложение). Свёрнуто последние 40 событий (`TIMELINE_COLLAPSED`).
    Названия и цвета статусов — из `globalBoardColumns(columns)` («Проверка»), остальные колонки проекта — запасом. `GlobalTask` запусков
    координатора не несёт (`Run.coordinatorSessions`), поэтому `App` передаёт их в `GlobalTaskView` отдельным необязательным пропсом
    `coordinatorSessions` из `snap.runs`. Блок «История статуса» (`StatusHistoryBlock`) из `GlobalTaskModal` убран — его заменила
    лента; в `TaskModal` подзадачи он остался. Стили — `.gt-hist*`.
  - Стили — блок `.gt-*` в `styles.css` (шапка — `.gt-head*`, история — `.gt-hist*`, вкладка «Координатор» — `.gt-coord-*`, корень `.gt-coord-panel`; `.gt-coord` и `.gt-coord-dot` — пилюля шапки, их не занимать).
  - **Шапка `GlobalTaskHeader.tsx`** (`.gt-head*`) — три строки, высота не зависит от состояния (описание и «обновлено» в шапке нет:
    цель — на «Итог и цель»). Логика — в `globalScreen.ts`, с тестами:
    1. крошка «← Глобальные задачи» (название задачи в крошках не дублируется) и **степпер статуса** Бэклог → В работе → Нужен ответ →
       Проверка → Сделано (`statusSteps` по `globalBoardColumns`, колонки как на глобальной доске). Клик по шагу — `globalTasks.move`
       (`App.moveGlobalTask`, тот же, что у перетаскивания на общей доске); «Нужен ответ» вычисляется по запросам, поэтому не кликается.
       Статус неизвестен (старый main, пользовательская колонка) — текущего шага нет. В окне ≤ 900px (`@media`) вместо ленты шагов —
       чип «Проверка ▾» с `MoveMenu` (цифра = номер колонки);
    2. заголовок и **одно главное действие по состоянию** (`headerActions`, условия «можно ли» — `globalTaskActions`, чтобы шапка и доска
       не расходились): «Проверка» — «✓ Подтвердить» + «Вернуть в работу…»; «Нужен ответ» — «Ответить · N `G`» (N — пункты ленты
       «Ждут вас», нет ленты — `GlobalTask.waiting`; клик фокусирует ленту, как клавиша G); координатор не запущен — «▶ Запустить
       координатора» (в «Сделано» — без акцента). Живой координатор работает сам — кнопки нет. «Изменить» и «⋯ → Удалить задачу…» —
       второстепенные;
    3. мета: приоритет, тип, **пилюля координатора** (`coordinatorPill`): «● Координатор работает · модель · N-й запуск · время
       · Терминал →», «ждёт вас» (карточка в «Нужен ответ»), «завершил», «не запущен». Модель, номер и время — из
       `Run.coordinatorSessions` (пробрасывает `App` из `snap.runs`; текущий запуск — по `ptyId` живого терминала, иначе последний);
       поля нет (старый main) — только агент из `GlobalTask.coordinatorAgent`, остальное пропущено. **«■ Остановить»** — подтверждение
       в самой пилюле («Остановить? Остановить / Отмена», Esc — отмена; не `confirm()`), затем `closeTerminal` → `pty.kill(coordinatorPtyId)`.
       Справа — время задачи (`GlobalDuration`).
- Лента «Ждут вас» (`AttentionFeed.tsx`, данные — `attention.ts` → `buildAttention`) стоит на экране глобальной задачи
  ...
  **G** — фокус в ленту (развернув её), а с фокусом в ленте — обратно на доску (выделенная карточка или первая). Не срабатывают
  ...
  (`revealOnBoard`) открывает доску. **Вкладки — `tabKey`** (тот же обработчик): **Alt+1…5** везде, где не идёт ввод и нет модалки; голые
  **1…5** — только вне доски, ленты и меню «Переместить в…» (`.board-wrap`, `.attn`, `[role="menu"]`): там цифры принадлежат им
...
- `GlobalStatsPanel.tsx` («Статистика»): итог (время жизни, своё время, агенты, «ждала вас», стоимость), полоса по колонкам, сравнение
  «Координатор / Подзадачи», топ подзадач (`byTask`, клик по названию открывает подзадачу), роли и возвраты с «Проверки» — по
  `stats:global`; подробности — `docs/architecture.md`, «Интерфейс — статистика задачи». У «Входящих» вкладки нет.
