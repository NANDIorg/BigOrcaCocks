...
| `git?` | ветка глобальной задачи `{branch, base, worktree?, pushedAt?, pushError?, prUrl?, prError?, prErrorCode?}` (`prUrl`/`prError` — PR ветки, который приложение открыло через `gh` после push при включённой настройке `pr`; `prErrorCode` — `ghMissing` / `ghAuth` / `other` для перевода в UI) — подзадачи ответвляются от неё и сливаются в неё (`src/main/run-branch.ts`, «Ветка глобальной задачи» в `docs/architecture.md`). Нет — «Входящие» (у них ветки не бывает), выключенная настройка проекта или прогон, начатый до веток: подзадачи сливаются в текущую ветку корня. Миграции нет: у старых прогонов поле просто не появляется |
