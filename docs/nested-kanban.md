...
- `GlobalTaskView.tsx` — экран открытой глобальной задачи: `GlobalTaskHeader` (компактная шапка, см. ниже),
  ...
  - `CoordinatorPanel.tsx` — заглушка вкладки «Координатор» (состояние координатора и кнопки; запуски и вывод появятся позже).
    Всё, что вкладки читают из снапшота (`coordinatorSessions`, `statusHistory`, `summary`, `returns`), необязательно — renderer
    работает со старым main.
  ...
  - Стили — блок `.gt-*` в `styles.css` (шапка — `.gt-head*`, история — `.gt-hist*`).
