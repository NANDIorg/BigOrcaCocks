...
| `git?` | ветка глобальной задачи `{branch, base, worktree?, pushedAt?, pushError?}` — подзадачи ответвляются от неё и сливаются в неё (`src/main/run-branch.ts`, «Ветка глобальной задачи» в `docs/architecture.md`). Нет — «Входящие» (у них ветки не бывает), выключенная настройка проекта или прогон, начатый до веток: подзадачи сливаются в текущую ветку корня. Миграции нет: у старых прогонов поле просто не появляется |
...
(`closeFinishedRuns`, `run_done` «все подзадачи в done», `settleIdleRuns`) для него выключено. Карточка следует за нодой: `work`, `ask`, `gate`, `git`, `merge` —
«В работе», `human` — «Проверка» (ждёт approval прогона; «Подтвердить» и «Вернуть в работу» решают его), `end` — прогон закрыт (`closedAt`), карточка в «Сделано»
