...
- `GlobalTaskView.tsx` — экран открытой глобальной задачи: `GlobalTaskHeader` (шапка, поведение прежнее: крошки «← Глобальные
  ...
  - `CoordinatorPanel.tsx` — вкладка «Координатор» (слева состояние и запуски, справа последний вывод; ≤ 900px — одна колонка):
    ...
    - Запуски — `Run.coordinatorSessions` (`snap.runs` → `GlobalTaskView.runs`), `sessionRows`: новые сверху, «Запуск N», агент и модель,
  ...
  - `GlobalHistory.tsx` — заглушка вкладки «История» (лента событий появится позже). Всё, что она будет читать из снапшота
    (`statusHistory`, `summary`), необязательно — renderer работает со старым main.
