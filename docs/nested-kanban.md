...
- **Воркфлоу прогона** (`workflowScope: 'run'`): вместо «Повторного запуска» цель — исходная цель и блок «# Этап «…»»
  (`runResumeCoordinatorObjective`, `coordinatorStageSection`): координатор — диспетчер и продолжает с этапа, где стоит граф (на этапах, которые ведёт приложение,
  ему сказано ждать `stage_started`/`run_done`). Приложение само перезапускает мёртвого координатора на входе в «Работу» (`workflow-run.ts`).
