...
- **Что сделал** — блок на экране глобальной задачи в «Проверке» (`GlobalTaskView.tsx`, выбор текста —
...
- `Board.tsx` (+ `BoardCard.tsx`, `MoveMenu.tsx`, `cardState.ts`, `boardView.ts`, `boardNav.ts`) — **локальная доска подзадач**
  ...
  действия — строка сути («? вопрос», «✕ Упал», «◉ Показ») и ссылка «в ленте ↑» (колбэк `onRevealInFeed?`, нет — нет ссылки).
  Тулбар: прогресс, фильтры «Все / Ждут вас / Проблемы / Мои роли», сортировка. «Ждут вас» = `waitsForYou` (вопрос, готовый ответ,
  сбой, ревью с показом или ответом). Клавиатура: стрелки, `Enter`, `M` (меню «Переместить в…», цифры 1–9), `S`. Названия этапов
...
- Лента «Ждут вас» (`AttentionFeed.tsx`, данные — `attention.ts` → `buildAttention`) стоит на экране глобальной задачи
  ...
  **Без дублей:** вопрос, по которому есть pending-запрос (`questionId`), — один пункт (запрос); эскалация-запрос и
  ...
  ленты нет совсем. Клавиша **G** (не в поле ввода, без модалки) — фокус в ленту, стрелки / Home / End — между
  карточками. Связка с доской: у карточки `data-task-id`, `revealAttention(taskId)` (событие `orca:attention-reveal`)
  разворачивает ленту, прокручивает к карточке и подсвечивает её на 2,5 с; необязательный `onSelectTask(taskId)` —
  клик по имени задачи в карточке ленты (выделить карточку на доске).
