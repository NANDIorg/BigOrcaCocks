...
- **Состояние карточки** (`renderer/src/cardState.ts`, чистые функции): `cardState` сводит вид колонки, последний dispatch
  ...
  фильтры тулбара. `waitsForYou` — «ждёт человека» для фильтра «Ждут вас»: `human`, `bad`, а `review` — только с показом
  (`Dispatch.showcase`) или ответом; ревью, которое проверяет гейт-агент, человека не ждёт. Лента «Ждут вас» считает то же.
