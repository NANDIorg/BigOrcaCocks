...
- **«Настройки»** (`settings/SettingsModal.tsx`): модальное окно по шестерёнке в rail (`showSettings` в `App.tsx`,
  ...
  - Встроенный шаблон (`builtin`) — просмотр, кроме **модели и усилия ролей**: `RolesEditor` с `modelOnly`
    (`templateRolesMode()` → `'models'`; id, название, назначение, агент, инструкции, добавление/удаление/порядок ролей
    заблокированы, правка роли проходит через `modelOnlyPatch`). Первая такая правка — обычный `patch(id, {roles})`:
    `patchedTemplate` из встроенного даёт копию с тем же id и явными ролями, main принимает её по `isBuiltinModelEdit`,
    и шаблон становится «изменённым встроенным» (дальше правится целиком; «Вернуть встроенный» удаляет копию).
    Старый main (прежний текст «…только для чтения — сделайте копию») → `BUILTIN_MODELS_STALE_MESSAGE`
    («перезапустите приложение», `templatesError`). `ColumnsEditor` — `readOnly`, холст воркфлоу не меняет граф,
    инспектор и прочие разделы — в `<fieldset disabled>` (`display: contents`, раскладку не трогает); баннер: модели —
    во вкладке «Роли», остальное — «Дублировать». Проекты этого типа не меняются — «Применить к проектам…».
...
- `saveTemplate({id?, title, description?, settings})` — создать (без `id` — `tpl_<hex>`) или целиком заменить
  ...
  Встроенный id без пользовательской копии сохраняется, только если отличаются лишь модель и усилие ролей
  (`isBuiltinModelEdit` в core: название, описание, прочие разделы, состав, порядок и остальные поля ролей — как у
  встроенного): так создаётся «изменённый встроенный» с тем же id. Иначе — ошибка «шаблон «…» встроенный и только для
  чтения: без копии в нём меняются только модель и усилие ролей, остальное — через «Дублировать»». Своя копия дальше
