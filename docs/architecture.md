...
- **Типы задач** (`packages/core/src/task-types.ts`, без node-импортов — для main и renderer). `TaskType {id, title,
  description?, builtin?, builtinBase?, settings: {roles?, workflow?, agentRules?, permissionMode?}}` (режим — `TaskTypePermissionMode`):
  ...
  - `builtinBase?` — только у изменённого встроенного (см. ниже): отпечаток системной версии, поверх которой сделана правка.
  - Встроенные — `builtinTaskTypes()` (свежие копии; по id — `builtinTaskType(id)`), названия — по виду задачи:
    ...
    projects.json. Название в коде — можно: копия встроенного без `builtinBase` получает новое название при загрузке
    (`migratedBuiltinOverride` в main).
  - Встроенный тип правится **целиком**, как пользовательский (название, описание, роли, граф, разрешения, правила); main
    сохраняет правку «изменённым встроенным» — пользовательским типом с тем же id. Удалить встроенный нельзя, удаление
    правки — «Сбросить к системному». `builtinTaskTypeFingerprint(id)` — отпечаток (FNV-1a от `stableJson` названия,
    описания и настроек из кода); `builtinTypeOutdated(t)` — правка сделана поверх другой системной версии, чем в текущем
    приложении. **Правка побеждает**: новая версия дефолта её не перетирает, UI только предлагает сброс.
    Снимок типа (`Run.taskType`) и граф (`Run.workflow`) уже созданных глобальных задач правка и сброс не меняют.
...
- **«Настройки»** (`settings/SettingsModal.tsx`): модальное окно по шестерёнке в rail (`showSettings` в `App.tsx`,
  ...
  - Группа «Типы задач» — каждый тип отдельным пунктом меню (`type:<id>` в `orca.settingsSection`; старые
    ...
    (форма в шапке: название и описание; есть и у встроенных), у своих — «Удалить», у изменённого встроенного —
    «Сбросить к системному» (`taskTypes:delete`). Подтверждение обоих — панель под шапкой, не `confirm()`: заголовок,
    последствия и надпись кнопки — `typeRemovalConfirm` (удаление: проекты перейдут на тип библиотеки по умолчанию,
    задачи доработают по снимку; сброс: проекты остаются на типе, граф задач прежний, роли — системные со следующего
    запуска). После сброса `rev` пересоздаёт редакторы — id тот же, значения системные. Ниже вкладки (`orca.settingsTypeTab`): «Роли», «Воркфлоу» (`settings/TaskTypeWorkflow.tsx` —
  ...
  - Встроенный тип (`builtin`) и его изменённая копия редактируются теми же редакторами, что свой тип, без блокировок.
    Первая правка — обычный `patch(id, …)` / `rename`: `patchedTaskType` из встроенного даёт тип с тем же id, и он
    становится «изменённым встроенным». Баннер говорит, что версия пользователя переживёт обновления, и — если
    `builtinTypeOutdated` — что встроенная версия с тех пор обновилась (взять её — «Сбросить к системному»).
    «Вернуть системные роли» в `RolesEditor` — про роли `DEFAULT_ROLES`, удалённые из списка; тип целиком возвращает
...
- Мусор при `load()`: `settings` не-объект, пустые id — отбрасываются. Тип отбрасывается только без `id`, названия или
  ...
  и `builtinBase` (`migratedBuiltinOverride`), если `builtinBase` нет: такая копия сделана до полной правки встроенных и
  название менять не могла, а встроенные могут переименовать в новой версии. С `builtinBase` название — правка
  пользователя. У пользовательских типов `builtinBase` снимается.
...
- `taskTypes()` — встроенные (`builtinTaskTypes()`, их порядок), затем пользовательские. Встроенные в файле не хранятся.
  Пользовательский тип с id встроенного **подменяет** его целиком («изменённый встроенный»); удаление копии —
  «Сбросить к системному».
...
- Встроенный тип правится `saveTaskType` целиком, с той же валидацией, что свой; результат — копия с его id и
  `builtinBase`. `builtinBase` ставится при первой правке (`builtinTaskTypeFingerprint`) и дальше переносится из
  сохранённой копии, вход его не задаёт: иначе любая следующая правка прятала бы, что системная версия обновилась.
- `patchTaskType(id, patch)` — мерж раздела (null у графа и разрешений — встроенное значение); `saveTaskTypeRules(typeId,
  ...
  `deleteTaskType(id)` — пользовательские (ссылки проектов остаются висячими и при чтении пропускаются) и изменённые
  встроенные — сброс к системному: id тот же, `defaultTaskTypeId` и `taskTypeIds` остаются в силе. Встроенный без
  правки — ошибка «тип «…» встроенный, его нельзя удалить…».
...
- **Id встроенного типа менять нельзя, название — можно, но с нормализацией копий.** Id — ссылка из
  ...
  от старой версии (миграция старого `defaults`, правка исполнителя до полной правки встроенных) хранит название на
  момент сохранения, и после переименования встроенных («Общий» → «Программирование») она осталась бы под старым:
  `migratedBuiltinOverride` в `load()` для копий без `builtinBase`
