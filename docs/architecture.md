...
- **Строки в модулях логики.** Экспорт-константа с текстом вычислялась бы один раз на языке загрузки, поэтому:
  ...
  `taskTypesStaleMessage()`, `staleAppMessage()`, `agentRulesPlaceholder()`). `STALE_APP_MESSAGE` оставлен
  русской константой только ради ещё не переведённых импортёров (`showcase.ts`, `statsFormat.ts`).
