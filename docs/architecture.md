...
- Не выключай действие человека до события, которое может не наступить. «Вернуть в работу…» на «Проверке»
  была выключена, пока жив терминал прежнего координатора, — «пара секунд после `runs finish`». Но Claude без
  `runs finish` терминал не закрывает, и поле для уточнения становилось недоступно насовсем. Теперь возврат
  закрывает прежнего координатора сам (`returnGlobalTaskToWork` в `src/main/coordinator-resume.ts`).
