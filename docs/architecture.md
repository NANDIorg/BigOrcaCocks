...
- **Разделы шаблона** (`packages/core/src/template-sections.ts`, без node-импортов — для main и renderer): основа
  «Типов проектов» (шаблоны вместо единственного дефолта). Раздел — `TemplateSection`: `agents`, `roles`, `columns`,
  `workflow`, `permissions`, `agentRules`; `SectionSettings` — эти поля у проекта, дефолта и шаблона, пустое поле —
  встроенное значение (`DEFAULT_ROLES`, `DEFAULT_COLUMNS`, `auto`, все установленные агенты, `defaultWorkflow(roles)`).
  - `sectionsDiff(project, template, agents)` — отличия по разделам (роли и колонки — добавленные/удалённые/изменённые
    по id и порядок; агенты — среди установленных; воркфлоу — исполняемый граф), `sectionDiffLine` — строка «Обзора».
  - `applySections(project, template, sections, roleIds?)` — итоговые настройки проекта, на диск не пишет. Раздел,
    которого в шаблоне нет, у проекта удаляется. `roleIds` — взять только эти роли (`mergeRole`: замена по id на месте,
    новая — в конец; нет в шаблоне — удаляется). Если применены роли, колонки или граф, итоговый граф проверяется
    `validateWorkflow` по итоговым ролям и колонкам; ошибки → исключение `после применения шаблона воркфлоу проекта
    ломается: …; примените вместе с разделами: …` (`appliedWorkflowErrors` — те же ошибки без исключения).
...
- **«О проекте»** (`about/AboutProject.tsx`): только настройки активного проекта (`projects:set*`, после —
  ...
  горизонтальной полосой. Шаблоны здесь не редактируются — только тип проекта, сравнение с шаблоном и выборочное применение в «Обзоре».
  - «Обзор» (`OverviewSection.tsx`) — статистика (задачи/открытые, терминалы, идущие прогоны, включённые агенты);
    ...
    «Скопировать»; блок «Тип проекта» (`about/ProjectTypeBox.tsx`, логика — `renderer/src/projectType.ts`):
    - «Тип: <шаблон>» — шаблон проекта (`Project.templateId`). База сравнения — `projectBase`: свой шаблон, а при
      пустом или висячем (шаблон удалён) `templateId` — шаблон по умолчанию с пояснением, как `baseTemplate` в main.
      Список шаблонов — хук `about/useProjectTemplates.ts` (`templates:list`), перечитывается при каждом открытии «Обзора».
    - Отличия от шаблона по разделам (`templateDiffRows` — `sectionsDiff` + `sectionDiffLine` из core; сравнивается
      исполняемый граф: свой или дефолтный по ролям), у каждой строки «Взять из шаблона», у ролей — ещё по одной роли
    ...
    - «Сохранить как шаблон…» (`about/SaveTemplateModal.tsx`, вместо прежнего «Сделать дефолтом») — настройки проекта
      (`projectAsTemplateSettings`: роли и колонки явно, свой граф — если есть) в новый пользовательский шаблон или
      поверх пользовательского (`templates:save`; встроенные только читаются, в списке их нет). Галочка «Сделать
      типом проекта» — затем `applyTemplate` всех разделов: настройки те же, меняется только `templateId`.
    - Старый preload без `templates` / `projects.applyTemplate` (`templatesApi()` → null) — прежний блок «Дефолт для
      новых проектов» (`about/defaultsDiff.ts`, «Сделать дефолтом» через `setDefaults`, «Применить дефолт…» через
      `projects:applyDefaults`). Старый main («No handler registered») — `TEMPLATES_STALE_MESSAGE` «перезапустите приложение».
...
- **«Настройки»** (`settings/SettingsModal.tsx`): модальное окно по шестерёнке в rail (`showSettings` в `App.tsx`,
  ...
  - Группа «Шаблоны проектов» — каждый шаблон отдельным пунктом меню (`tpl:<id>` в `orca.settingsSection`; старые
    значения `agents`/`roles`/… ведут на шаблон по умолчанию): сначала встроенные (изменённый встроенный — на месте
    своего встроенного, `splitTemplates`), затем «Свои», внизу «Новый шаблон»
    ...
    `deleteConfirmText`; у изменённого встроенного кнопка называется «Вернуть встроенный»). Ниже вкладки разделов (`orca.settingsTemplateTab`): «Агенты», «Роли», «Колонки»,
  ...
  - Встроенный шаблон (`builtin`) — просмотр, кроме **исполнителя ролей — агента, модели и усилия**: `RolesEditor`
    с `executorOnly` (`templateRolesMode()` → `'executor'`; id, название, назначение, инструкции,
    добавление/удаление/порядок ролей заблокированы, правка роли проходит через `executorOnlyPatch`). Смена агента —
    та же правка, что в полном редакторе (`agentChangePatch` в `renderer/src/roleEdit.ts`): модель и усилие
    прошлого агента сбрасываются в «по умолчанию», выбор модели/усилия дальше — по новому агенту. Первая такая правка — обычный `patch(id, {roles})`:
    `patchedTemplate` из встроенного даёт копию с тем же id и явными ролями, main принимает её по `isBuiltinExecutorEdit`,
    и шаблон становится «изменённым встроенным» (дальше правится целиком; «Вернуть встроенный» удаляет копию).
    Старый main (прежние тексты «…только для чтения — сделайте копию» и «…меняются только модель и усилие ролей» —
    main до смены агента) → `BUILTIN_EXECUTOR_STALE_MESSAGE`
    («перезапустите приложение», `templatesError`). `ColumnsEditor` — `readOnly`, холст воркфлоу не меняет граф,
    инспектор и прочие разделы — в `<fieldset disabled>` (`display: contents`, раскладку не трогает); баннер: агент, модель
    и усилие — во вкладке «Роли», остальное — «Дублировать». Проекты этого типа не меняются — «Применить к проектам…».
  - Хук `settings/useTemplates.ts`: список `templates:list`, после каждой записи перечитывается целиком, плюс
    `refreshProjectDefaults()` (`about/useProjectDefaults.ts`) — шаблон по умолчанию читают «Обзор» и ассистент.
    `templates:save` заменяет шаблон целиком, а редакторы сохраняются с задержкой, поэтому правка раздела
    (`patch(id, patch)` → `patchedTemplate`, null удаляет поле) собирается из последней сохранённой версии шаблона
    и идёт через очередь — отложенное сохранение ролей не затирает только что выбранный режим разрешений.
  - Логика без React — `renderer/src/projectTemplates.ts` (тест рядом): встроенные значения незаданных разделов,
    сборка `TemplateInput`, «используют N проектов» по `Project.templateId` (проекты без поля не считаются),
...
- **Редакторы ролей/колонок** (`RolesEditor`, `ColumnsEditor`) не знают о проекте: `storageKey` (ключ `useAutoSave`) + начальные `roles`/`columns` + `onSave`, `readOnly` — только просмотр. В «О проекте» `storageKey = active.id`, в «Настройках» — `templateEditorKey()`: `tpl:<id>:b|u` (признак встроенного — чтобы после удаления изменённого встроенного шаблона редакторы сбросили черновик удалённой копии). Редактор ролей — `rolesEditorKey(t, promoted)`: встроенный, ставший копией из этого редактора (смена агента или модели), сохраняет ключ `b`, иначе ответ main сбросил бы черновик посреди быстрых кликов; перед «Вернуть встроенный» и при смене шаблона `promoted` сбрасывается, и ключ меняется как обычно. `executorOnly` — меняются только агент, модель и усилие.
...
- IPC, заменяющий объект целиком (`templates:save`), плохо сочетается с автосохранением редакторов с задержкой
  (`useAutoSave`, 300 мс): колбэк, захваченный при вводе, собрал бы шаблон из старой версии и затёр правку
  соседнего раздела, сделанную за это время. Собирай запись из последней сохранённой версии и сериализуй записи
  (`update` в `settings/useTemplates.ts`).
- **Всё, что читается из `projects.json`, проверяй той же валидацией, что при сохранении.** `loadedTemplate`
  проверял только id/название/объект настроек: руками испорченный шаблон без колонки backlog давал проект без
  backlog, а `applyTemplate` успевал записать правила и сохранить файл до ошибки `setColumns`. Теперь настройки
  шаблона идут через `validSettings` (битый шаблон отбрасывается целиком), а `applyTemplate` проверяет роли и
  колонки до первой записи. Многошаговая запись — сначала проверить всё, потом писать.
- Строка CSS-сетки с необязательными элементами держит колонки только явным `grid-column`. В `.roles-item`
  (ручка или точка — кнопка — чип) у роли для задач в шаблоне «только чтение» нет ручки, и кнопка вставала
