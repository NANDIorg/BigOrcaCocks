...
- **«О проекте»** (`about/AboutProject.tsx`): слева меню разделов, справа один раздел (выбранный хранится в
  `localStorage` `orca.aboutSection`). Над меню переключатель «Этот проект / Для новых проектов» (`AboutScope`,
  состояние в `App.tsx`): те же разделы редактируют либо активный проект (`projects:set*`, после — `refreshProjects`),
  либо дефолт для новых проектов (`projects:getDefaults` / `setDefaults(patch)`). В режиме дефолта нет «Обзора»
  и «Прогонов» (открыт один из них — показываются «Агенты»), сверху баннер; без активного проекта доступен только
  дефолт. Шестерёнка в rail открывает вкладку сразу в режиме «Для новых проектов». Отдельной модалки дефолта нет.
  У пунктов меню счётчики: агенты «N из M» (включено из установленных), роли (+ «k !» — роли с выключенным
  агентом), колонки, режим разрешений, прогоны («N идёт» или всего). Заголовки разделов — общий `SectionHead`
  (`about/parts.tsx`). Узкая вкладка (`@container about`, ≤ 900px) — меню становится горизонтальной полосой.
  ...
  - «Агенты» (`AgentsSection.tsx`) — карточки установленных (логотип, название, версия) с переключателем
    (`enabledAgents`), не установленные — под спойлером; «Обновить» пересканирует PATH. В дефолте есть
  ...
  - «Приложение → Общие» (`AppSection.tsx`) — глобальные настройки приложения («Работать в фоне»), в обоих режимах.
...
- **Настройка** `AppSettings.keepInBackground` (`shared/ipc.ts`) — глобальная, не проектная: поле `settings`
  ...
  переключатель «Работать в фоне при закрытии окна» в разделе «Приложение → Общие» вкладки «О проекте»
  (`about/AppSection.tsx`), IPC `app:getSettings` / `app:setSettings`.
