...
import {
  ...
  type OrcaEvent, type RunBranchSettings, type Task, type WfEdge, type WfNode, type Workflow
...
beforeEach(() => {
  ...
  deps = {
    ...
    // Как runWorker + startWorker: ветка задачи от ветки прогона, worktree, dispatch с ролью задачи.
    startWorker(taskId) {
      const t = task(taskId)
...
function resolve(requestId: string, action: 'accept' | 'reject', text?: string): void {
  resolveHumanRequest(store, repo, requestId, { action, ...(text ? { text } : {}) }, deps.startWorker, (r) => handleRunApproval(deps, r), deps.mergeTarget)
}
...
describe('подзадачи: автомерж и конфликт', () => {
  it('конфликт мержа подзадачи → approval «Конфликт мержа» на задаче; «Принять» после правки — мерж и done', () => {
    ...
    assert.equal(request.nodeId, SUBTASK_MERGE_NODE)
