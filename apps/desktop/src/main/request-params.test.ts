...
describe('resolutionFromParams', () => {
  ...
  it('accept с решением, clarify, restart, dismiss', () => {
    ...
    assert.deepEqual(resolutionFromParams(req, { reject: 'поправь тесты' }), { action: 'reject', text: 'поправь тесты' })
    assert.throws(() => resolutionFromParams(req, { reject: true }), /требует значения/)
    assert.throws(() => resolutionFromParams(req, { accept: true, reject: 'x' }), /укажи одно/)
