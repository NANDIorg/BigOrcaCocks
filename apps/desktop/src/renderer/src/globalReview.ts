...
export interface GlobalTaskActions {
  ...
  accept: boolean
  /**
   * «Вернуть в работу…»: с «Проверки» в работу с уточнением и повторным запуском координатора. Доступна всегда,
   * даже при живом прежнем координаторе: раньше кнопка в этом случае выключалась, а терминал Claude без
   * `runs finish` не закрывается сам — поле для уточнения становилось недоступно насовсем.
   */
