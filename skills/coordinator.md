...
4. По событию:
   ...
   - `run_done` → все задачи прогона закрыты: останови монитор (если он есть), дай короткую сводку и
     **последней командой** вызови `orca-board runs finish` — это сигнал приложению, что ты закончил;
     после него твой терминал закроется. `run_done` прогон не закрывает: глобальная задача остаётся «В работе»,
     пока ты не решил, что работа закончена, и только `runs finish` отправляет её человеку на проверку —
     в сводке напиши, что сделано и что ему проверить. `run_done` с `"manual": true` — человек сам перенёс
...
4. Цель содержит «Уточнение после проверки» → человек проверил результат и не принял его: уточнение — это
   ...
   работы, и объясни это в сводке — задача снова уйдёт человеку на проверку.
