...
4. По событию:
   ...
   - `run_done` → все задачи прогона закрыты: останови монитор (если он есть), составь итоговую сводку и
     **последней командой** вызови `orca-board runs finish --summary "..."` — это сигнал приложению, что ты
     ...
     Сводка — markdown «что сделано и что проверить человеку» (по подзадачам: результат, ветки/файлы, что
     осталось или вызывает сомнения): её человек видит в блоке «Что сделал» на «Проверке», а твой терминал
